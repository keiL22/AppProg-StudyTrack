...
public class AssignmentViewController {
    ...
    void addPressed(ActionEvent event) {
    	if(nameAdd.getText()!=null&&dateAdd.getValue()!=null) {
    		Assignment addedStudent = new Assignment(nameAdd.getText(), dateAdd.getValue().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")), CourseViewController.courses.get(CourseViewController.lastCourseClicked), false); //creasting a new assignment object based on the user input    		//storedAssigns.add(addedStudent); //adding to the arraylist, will need to make a case statement later since there will be multiple array lists (one for each possiblee course created)
    ...
    public void createTable() {
        ...
        editCol.setCellFactory(param -> new TableCell<Assignment, Assignment>() {
        	ImageView deleteView = new ImageView("/data/DeleteIcon.png");
            Button deleteButton = new Button("", deleteView);//new button that will be in column
            ImageView startView = new ImageView("/data/StartIcon.png");
            ...
            protected void updateItem(Assignment assign, boolean empty) {
                ...
                setGraphic(empty ? null : buttons);//making sure it not null or it'll mess up
                EventHandler<ActionEvent> startPress = new EventHandler<ActionEvent>() {//making new event handler for the start button
                	//Takes the user to the TimerPage and start the timer
                	@Override
                ...
                EventHandler<ActionEvent> deletePress = new EventHandler<ActionEvent>() {//making new event handler for delete button
                	@Override
                	public void handle(ActionEvent event) {
                		currentCourse.storedAssigns.remove(assign);
                	}
                };
        ...
        assignmentsTabel.setItems(currentCourse.storedAssigns);//another case statement thing you get the list, this just updates the table with all the items in the array list
    }
}
